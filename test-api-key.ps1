Write-Host "🧪 API Key Testing Tool (PowerShell)" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Write-Host "1️⃣ Creating a new API key..." -ForegroundColor Yellow
    ...
    Write-Host "2️⃣ Testing API key..." -ForegroundColor Yellow
    ...
    Write-Host "✅ API Key works! Found $($keysResponse.Count) keys" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "4️⃣ Testing invalid API key..." -ForegroundColor Yellow
    try {
        $invalidHeaders = @{
            "Authorization" = "Bearer invalid_key"
        }
        Invoke-RestMethod -Uri "$baseUrl/api/auth/keys" -Method Get -Headers $invalidHeaders
        Write-Host "❌ Invalid key test failed - should have been rejected" -ForegroundColor Red
    }
    catch {
        if ($_.Exception.Response.StatusCode -eq 401) {
            Write-Host "✅ Invalid key correctly rejected" -ForegroundColor Green
        }
        else {
            throw
        }
    ...
    Write-Host ""
    Write-Host "📋 Your working API key: $apiKey" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "💾 API key saved to api-key.txt" -ForegroundColor Green
    ...
    if ($_.Exception.Message -like "*refused*") {
        Write-Host ""
        Write-Host "💡 Make sure your server is running on port 3001" -ForegroundColor Yellow
        Write-Host "Run: npm run dev (in the server directory)"
    }
}
